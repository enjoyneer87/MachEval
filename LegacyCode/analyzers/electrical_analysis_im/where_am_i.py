...
def where_am_i_v2(fea_config_dict, bool_post_processing=False):
    ...
    dir_codes3 = os.path.dirname(__file__) + '/'

    dir_parent = os.path.realpath(dir_codes3 + '../') + '/' # convert relative path to absolute path

    # if not bool_post_processing:
        # if 'codes3' not in dir_codes3:
        #     raise Exception('Do not run the script from your current directory: %s.\nPlease cd to ./codes3 before running python scripts.'%(dir_codes3))
    # dir_codes = dir_parent + 'codes3/'
    # dir_femm_files = dir_parent + 'femm_files/'
    pc_name = get_pc_name()
    os.chdir(dir_codes3)
    # print(dir_parent, dir_codes, pc_name, sep='\n'); quit()
    fea_config_dict['dir.interpreter']       = dir_codes3 
    fea_config_dict['dir.parent']            = dir_parent
    fea_config_dict['dir.codes']             = dir_codes3
