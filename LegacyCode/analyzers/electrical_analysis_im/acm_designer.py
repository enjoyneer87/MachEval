...
class swarm_data_container(object):
    def __init__(self, swarm_data_raw, fea_config_dict, bound_filter):
        ...
        self.l_rated_rotor_weight = [(V * 8050 * 9.8) for V in
                                     self.l_rated_rotor_volume]  # density of rotor is estimated to be that of steraw of 8050 g/cm^3
        ...
        self.l_original_rotor_weight = [weight / rated * ori for weight, rated, ori in
                                        zip(self.l_rated_rotor_weight, self.l_rated_stack_length,
                                            self.l_original_stack_length)]

        required_torque = 50e3 / (
                    30000 / 60 * 2 * np.pi)  # TODO: should use rated stack length and torque average to compute this
    ...
    def get_x_denorm_from_design_parameters(self, design_parameters, bound_filter=None):
        ...
        if len(bound_filter) == 13:
            ...
            free_variables[3] = sum([design_parameters[i] for i in (2, 4, 5,
                                                                    6)])  # spmsm_template.mm_r_si + spmsm_template.mm_d_sp + spmsm_template.mm_d_st + spmsm_template.mm_d_sy # stator outer radius
            free_variables[4] = design_parameters[7]  # spmsm_template.mm_w_st
            ...
            free_variables[10] = sum([design_parameters[i] for i in (18, 17,
                                                                     19)])  # spmsm_template.mm_r_ri + spmsm_template.mm_d_ri + spmsm_template.mm_d_rp -> rotor_outer_steel_radius
    ...
    def sensitivity_bar_charts(self):
        ...
        mpl.rcParams['font.family'] = ['sans-serif']  # ['Times New Roman']
        ...
        for i in range(len(list_y_label)):
            ...
            for j in range(number_of_free_variables):
                ...
                ax.text(0.33 * number_of_free_variables + j * number_of_variant, high - (high - low) * 0.125,
                        free_param_list[j])
...
class FEA_Solver:
    def __init__(self, fea_config_dict, spec_input_dict):
        ...
        if fea_config_dict['bool_post_processing'] == False:
            self.fig_main, self.axeses = plt.subplots(2, 2, sharex=True, dpi=150, figsize=(16, 8), facecolor='w',
                                                      edgecolor='k')
    ...
    def write_swarm_survivor(self, pop, counter_fitness_return):
        with open(self.output_dir + 'swarm_survivor.txt', 'a') as f:
            f.write('\n---------%d\n' % (counter_fitness_return) \
                    + '\n'.join(','.join('%.16f' % (x) for x in el[0].tolist() + el[1].tolist()) for el in
                                zip(pop.get_x(), pop.get_f())))  # convert 2d array to string
    ...
    def fea_bearingless_spmsm(self, spmsm_template, x_denorm, counter, counter_loop, bool_re_evaluate=False):
        ...
        if bool_re_evaluate == False:
            ...
            study = spmsm_variant.add_magnetic_transient_study(app, model, self.dir_csv_output_folder,
                                                               study_name)  # Change here and there
            ...
            if self.fea_config_dict['delete_results_after_calculation'] == False:
                ...
                app.GetDataManager().GetGraphModel("Circuit Voltage").WriteTable(
                    self.dir_csv_output_folder + spmsm_variant.name + "_EXPORT_CIRCUIT_VOLTAGE.csv")
        else:
            ...
            CurrentAmp_in_the_slot = THE_mm2_slot_area * spmsm_variant.fill_factor * spmsm_variant.Js * 1e-6 * np.sqrt(
                2)  # /2.2*2.8
            ...
            spmsm_variant.DriveW_CurrentAmp = spmsm_variant.fea_config_dict[
                                                  'TORQUE_CURRENT_RATIO'] * variant_DriveW_CurrentAmp
            spmsm_variant.BeariW_CurrentAmp = spmsm_variant.fea_config_dict[
                                                  'SUSPENSION_CURRENT_RATIO'] * variant_DriveW_CurrentAmp

            slot_area_utilizing_ratio = (
                                                    spmsm_variant.DriveW_CurrentAmp + spmsm_variant.BeariW_CurrentAmp) / spmsm_variant.CurrentAmp_per_phase
        ...
        ################################################################
        spmsm_variant.results_to_be_unpacked = results_to_be_unpacked = utility.build_str_results(self.axeses,
                                                                                                  spmsm_variant,
                                                                                                  self.project_name,
                                                                                                  study_name,
...
class acm_designer(object):
    ...
    def evaluate_design_json_wrapper(self, acm_template, x_denorm=None, counter=999, counter_loop=1):
        ...
        big_dict = dict()
        with open(self.output_dir + self.select_spec + ' gen%d-ind%d' % (
        number_current_generation, individual_index) + '.json', 'w') as f:
            big_dict['Inputs'] = acm_template.spec_input_dict
            big_dict['Derived'] = self.spec.spec_derive_dict
            big_dict['Geometry'] = motor_design_variant.spec_geometry_dict
            big_dict['Performance'] = spec_performance_dict
    ...
    def build_refined_bounds(self, the_bounds):
        ...
        print(
            'fea_config_dict - use_weights changed from %s to %s' % (self.fea_config_dict['use_weights'], buf_list[1]))
        ...
        for bnd1, bnd2 in zip(self.de_config_dict['original_bounds'], self.de_config_dict['narrow_bounds_normalized']):
            diff = bnd1[1] - bnd1[0]
            self.de_config_dict['bounds'].append(
                [bnd1[0] + diff * bnd2[0], bnd1[0] + diff * bnd2[1]])  # 注意，都是乘以original_bounds的上限哦！
    ...
    def best_design_by_weights(self, use_weights):
        ...
        def combine_lists_alternating_2(list1, list2):
            if abs(len(list1) - len(list2)) <= 1:
                ...
                raise Exception(
                    'Try this (not tested).')  # https://stackoverflow.com/questions/3678869/pythonic-way-to-combine-two-lists-in-an-alternating-fashion
                import itertools
        ...
        os.system(
            'cd /d ' + r'"D:\OneDrive - UW-Madison\c\release\OneReport\BestReport_TEX" && z_nul"')  # 必须先关闭文件！否则编译不起来的
        # import subprocess
    ...
    def run_static_structural_fea(self, design_denorm):
        ...
        if not os.path.exists(expected_project_file):
            ...
            logger.debug('Existing models of %d are found in %s', sw.app.NumModels(),
                         sw.app.GetDefaultModelFolderPath())

        # draw the model in JMAG Designer
        DRAW_SUCCESS = sw.draw_jmag_induction(-1,
                                              im_best,
                                              'Best %s %s' % (
                                              fea_config_dict['use_weights'], fea_config_dict['run_folder'][:-1]),
